...
static char(*fperm)[256][8]; /*[%] 8 * 256 * 8byte:
							...
							from 4-bit original input
							*/
static unsigned char(*kn)[8];/* 8 6-bit subkeys for each of 16 rounds, initialized by des_set_key() */
...
static long32 f(ulong32 r, unsigned char subkey[8])
{
	...
	//复制r到s, 即把r当成4个字节看待
	unsigned char *xs = (unsigned char *)&r;
	for (i = 4; i > 0; i--) {
		s[i - 1] = *xs;
		xs++;
	}

	/*根据plaintext_32bit_expanded_to_48bit_table这张表, 把s中的4字节共32位扩展成48位并
	...
	for (i = 0; i < 8; i++) {
		for (j = 0; j < 6; j++) {
			b = plaintext_32bit_expanded_to_48bit_table[6 * i + j] - 1;
			fm = b & 7;
			if (s[b >> 3] & bytebit[fm]) {
	...
	/*把plaintext中的8个字节与subkey中8个字节异或, 异或后的值保存到plaintext内;
	*该过程使用8次循环来做。
	*/
	for (i = 0; i < 8; i++) {
		plaintext[i] ^= subkey[i] & 0x3F;
	}

	/*取出plaintext[i]查表sbox[i]得4位数, 循环8次可得8个4位数, 按从左到右顺序合并
	*得32位数保存到数组m中。
	*/
	for (i = 0; i < 8; i++) {
		j = sbox[i][(plaintext[i] & 0x20) | ((plaintext[i] >> 1) & 0x0F) | ((plaintext[i] & 1) << 4)];
		m[i / 2] |= j << ((i % 2) ? 0 : 4);
	}

	/*根据sbox_perm_table把m中的32位数打乱保存到数组t中, 该过程需要使用32次循环来做,
    *每次循环提取1位。
	*/
	for (i = 0; i < 32; i++) {
		b = (sbox_perm_table[i] - 1);
		fm = b & 7;
		if (m[b >> 3] & bytebit[fm]) {
			t[i >> 3] |= bytebit[i & 7];
		}
	...
	//复制t到rval(注意大小端), 并返回rval
	unsigned char *xrval = (unsigned char *)&rval;
	for (i = 0; i < 4; i++) {
		xrval[i] = t[3 - i];
	}
	return rval;		
